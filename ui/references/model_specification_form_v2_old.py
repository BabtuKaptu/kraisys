...
class ModelSpecificationFormV2(QDialog):
    ...
    def create_cutting_tab(self):
        ...
        info = QLabel(
            ...
            "Выберите деталь из справочника и укажите примечания (материал, особенности обработки)"
        )
        ...
        self.cutting_part_combo.setMinimumWidth(300)
        ...
        self.cutting_notes_input.setPlaceholderText("Примечания (материал, обработка)")
        self.cutting_notes_input.setMinimumWidth(300)
        ...
        self.cutting_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
    ...
    def create_hardware_tab(self):
        ...
        # Количество/размер
        self.hardware_spec_input = QLineEdit()
        self.hardware_spec_input.setPlaceholderText("Кол-во/размер (150 см, 8 шт)")
        self.hardware_spec_input.setMinimumWidth(150)
        add_layout.addWidget(QLabel("Спецификация:"))
        add_layout.addWidget(self.hardware_spec_input)

        # Примечания
        self.hardware_notes_input = QLineEdit()
        self.hardware_notes_input.setPlaceholderText("Дополнительные примечания")
        self.hardware_notes_input.setMinimumWidth(250)
        add_layout.addWidget(QLabel("Примечания:"))
    ...
    def add_cutting_part(self):
        ...
        # Деталировка
        self.cutting_table.setItem(row, 0, QTableWidgetItem(f"{part_data['code']} - {part_data['name']}"))

        # Количество
        self.cutting_table.setItem(row, 1, QTableWidgetItem(str(self.cutting_qty_spin.value())))

        # Примечания
        notes = self.cutting_notes_input.text()
        if not notes and part_data.get('notes'):
            notes = part_data['notes']
        self.cutting_table.setItem(row, 2, QTableWidgetItem(notes))
    ...
    def add_hardware_item(self):
        ...
        # Наименование
        self.hardware_table.setItem(row, 0, QTableWidgetItem(f"{hw_data['code']} - {hw_data['name']}"))

        # Спецификация
        spec = self.hardware_spec_input.text()
        if not spec:
            # Автоматические подсказки по типу фурнитуры
            if "шнур" in hw_data['name'].lower():
                spec = "150 см"
            elif "крюч" in hw_data['name'].lower():
                spec = "8 крючков"
            elif "петл" in hw_data['name'].lower() or "блоч" in hw_data['name'].lower():
                spec = "20 шт на пару"

        self.hardware_table.setItem(row, 1, QTableWidgetItem(spec))

        # Примечания
        self.hardware_table.setItem(row, 2, QTableWidgetItem(self.hardware_notes_input.text()))
        ...
        self.hardware_spec_input.clear()
    ...
    def save_model(self):
        ...
        try:
            ...
            for row in range(self.cutting_table.rowCount()):
                ...
                notes = self.cutting_table.item(row, 2).text()

                # Извлекаем расход из примечаний если есть
                consumption = 0
                if 'Расход:' in notes:
                    try:
                        consumption = float(notes.split('Расход:')[1].split('дм²')[0].strip())
                    except:
                        pass
            ...
            for row in range(self.hardware_table.rowCount()):
                ...
                specification = self.hardware_table.item(row, 1).text()
                notes = self.hardware_table.item(row, 2).text()
                ...
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (self.model_id, component_name.split(' - ')[-1] if ' - ' in component_name else component_name,
                      'material', specification, f"{specification} {notes}".strip(), sort_order))
                sort_order += 1
    ...
    def load_model_data(self):
        ...
        try:
            ...
            for comp in components:
                ...
                # Загрузка деталей кроя
                if spec['cutting_parts']:
                    parts = json.loads(spec['cutting_parts']) if isinstance(spec['cutting_parts'], str) else spec['cutting_parts']
                    for part in parts:
                        row = self.cutting_table.rowCount()
                        self.cutting_table.insertRow(row)
                        self.cutting_table.setItem(row, 0, QTableWidgetItem(part.get('name', '')))
                        self.cutting_table.setItem(row, 1, QTableWidgetItem(str(part.get('quantity', 2))))
                        self.cutting_table.setItem(row, 2, QTableWidgetItem(part.get('notes', '')))

                        delete_btn = QPushButton("🗑")
                        delete_btn.clicked.connect(lambda r=row: self.cutting_table.removeRow(r))
                        self.cutting_table.setCellWidget(row, 3, delete_btn)

                # Загрузка фурнитуры
                if spec['hardware']:
                    hardware = json.loads(spec['hardware']) if isinstance(spec['hardware'], str) else spec['hardware']
                    for hw in hardware:
                        row = self.hardware_table.rowCount()
                        self.hardware_table.insertRow(row)
                        self.hardware_table.setItem(row, 0, QTableWidgetItem(hw.get('name', '')))
                        self.hardware_table.setItem(row, 1, QTableWidgetItem(hw.get('specification', '')))
                        self.hardware_table.setItem(row, 2, QTableWidgetItem(hw.get('notes', '')))

                        delete_btn = QPushButton("🗑")
                        delete_btn.clicked.connect(lambda r=row: self.hardware_table.removeRow(r))
                        self.hardware_table.setCellWidget(row, 3, delete_btn)

                # Загрузка вариантов
                if spec['variants']:
                    variants = json.loads(spec['variants']) if isinstance(spec['variants'], str) else spec['variants']

                    if 'perforation' in variants:
                        self.perf_text.setPlainText('\n'.join(variants['perforation']))

                    if 'lining' in variants:
                        self.lining_text.setPlainText('\n'.join(variants['lining']))

                    if 'sole' in variants:
                        for sole in variants['sole']:
                            row = self.sole_table.rowCount()
                            self.sole_table.insertRow(row)
                            self.sole_table.setItem(row, 0, QTableWidgetItem(sole.get('name', '')))
                            self.sole_table.setItem(row, 1, QTableWidgetItem(sole.get('size_range', '')))
                            self.sole_table.setItem(row, 2, QTableWidgetItem(sole.get('notes', '')))

                            delete_btn = QPushButton("🗑")
                            delete_btn.clicked.connect(lambda r=row: self.sole_table.removeRow(r))
                            self.sole_table.setCellWidget(row, 3, delete_btn)

                    if 'other' in variants:
                        self.other_variants_text.setPlainText('\n'.join(variants['other']))
